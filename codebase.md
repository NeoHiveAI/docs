...
- A Repo Index lives inside a Hive. [Create a Hive first](getting-started.md) if you haven't.
...
2. Navigate to your Hive and create a new **Repo Index**.
...
Once a Repo Index exists, NeoHive keeps it in sync on a schedule. Syncs are incremental, so they stay fast:
...
Every sync is recorded in the Index's sync history with its status, duration, and the range of commits it covered. That history is the first place to look when a sync fails or a change you expected isn't showing up.
...
Repo Indexes are indexed with a **code-tuned embedding model**, one trained on source code, so it matches "how does the sync engine retry?" to the right function even when your exact words never appear in it. (Knowledge and Document Indexes use a general-purpose text model instead.)
...
You don't have to work this out yourself. An Index's **Index Info** tab offers a choice of models, and each option is marked as a fit or not for your machine's available memory, so you can avoid picking one that won't load. Changing an Index's model re-indexes its contents, with live progress while it runs.

### Writing a good Index description

An Index's description isn't just a label. It's what your agent reads (through `list_indexes`) to decide _which_ Index to search for a given question. A vague description leaves the agent guessing; a specific one routes the right questions to the right Index.
...
With several Indexes connected, say a backend repo, a frontend repo, and a Knowledge Index, good descriptions are what let your agent reach for the backend Index on a payments question and the frontend Index on a rendering one, without you telling it where to look.

## Privacy & security

Everything stays on the machine running NeoHive. Repositories are cloned and indexed locally, embeddings live in your Hive's local data directory, and repository credentials are encrypted at rest. Nothing is sent to an external service to be indexed.
