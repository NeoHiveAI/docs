...
<CardGrid>
  <Card title="Repo hives" icon="github">
    Indexed source code from a GitHub or GitLab repository — this is how your agent searches your actual code by meaning, not by filename or keyword. See [Indexing Your Codebase](/codebase/).
  </Card>
  <Card title="Document hives" icon="document">
    Uploaded files: markdown, text, PDFs. Use these for content that doesn't live in git — runbooks, onboarding guides, design docs. Drag and drop them into the dashboard.
  </Card>
  <Card title="Knowledge hives" icon="star">
    Where your agents store and retrieve learned context — conventions, decisions, insights, corrections. Created automatically with the project; your agents read and write here as you work.
  </Card>
</CardGrid>
...
1. **Your agent sends a query.** Usually through the `memory_recall` or `memory_context` tools, phrased as a natural description of what it needs — e.g. *"error handling in the payment webhook retry path."*
...
3. **Each hive runs a hybrid search.** NeoHive combines **dense vector search** (meaning) with **BM25 keyword search** (exact terms) and fuses the two with **Reciprocal Rank Fusion** — so you get semantic recall *and* precise term matches, not one at the expense of the other.
...
This is also why *phrasing matters*. Because search is by meaning, describing the situation in the terms you'd expect to see in the answer gets the best results — see [Getting better results](/usage/#getting-better-results) for how to ask.
