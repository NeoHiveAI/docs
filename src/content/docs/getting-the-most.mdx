...
<li>Before we start — what do we already know about the auth flow here?</li>
<li>Have we hit flaky failures in the batch processor before? What fixed it?</li>
<li>Remember that we always use idempotency keys on payment POST requests.</li>
<li>That's out of date — we moved off Redis for sessions last month. Update it.</li>
<li>I'm switching to the search-indexing work now — pull up anything relevant.</li>
</ul>
